...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    elif num_classes == 1 or dataset == 'table':
  ...
  def add_coco_bbox(self, bbox, cat, conf=1, show_txt=False, img_id='default'): 
    ...
    cv2.rectangle(
      self.imgs[img_id], (bbox[0], bbox[1]), (bbox[2], bbox[3]), (0,0,255), 10) # -2
    if show_txt:
      ...
      cv2.putText(self.imgs[img_id], txt, (bbox[0], bbox[1] - 2), 
                  font, 10, (0, 0, 0), thickness=1, lineType=cv2.LINE_AA)

  def add_4ps_coco_bbox(self, bbox, cat, conf, logi=None, show_txt=False, img_id='default'): 
    bbox = np.array(bbox, dtype=np.int32)
    # cat = (int(cat) + 1) % 80
    cat = int(cat)
    # print('cat', cat, self.names[cat])
    ...
    xmin,ymin = min([bbox[0],bbox[2],bbox[4],bbox[6]]), min([bbox[1],bbox[3],bbox[5],bbox[7]])
    xmax,ymax = max([bbox[0],bbox[2],bbox[4],bbox[6]]), max([bbox[1],bbox[3],bbox[5],bbox[7]])
    if not logi is None:
      cv2.rectangle(self.imgs[img_id],
                    (bbox[0], bbox[1] - cat_size[1] - 2),
                    (bbox[0] + cat_size[0], bbox[1] - 2), (0,255,0), -1)
      cv2.putText(self.imgs[img_id], txt, (bbox[0], bbox[1] - 2), 
  ...
  def add_corner(self, corner, img_id='default'):
    if abs(int(corner[2])+int(corner[3])) > 10:
        cv2.arrowedLine(self.imgs[img_id], (int(corner[0]),int(corner[1])),(int(corner[2]),int(corner[3])),(0,0,255),thickness=1, line_type=cv2.LINE_4, shift=0, tipLength=0.02)
    if abs(int(corner[4])+int(corner[5])) > 10:
        cv2.arrowedLine(self.imgs[img_id], (int(corner[0]),int(corner[1])),(int(corner[4]),int(corner[5])),(0,255,0),thickness=1, line_type=cv2.LINE_4, shift=0, tipLength=0.02)
    if abs(int(corner[6])+int(corner[7])) > 10:
        cv2.arrowedLine(self.imgs[img_id], (int(corner[0]),int(corner[1])),(int(corner[6]),int(corner[7])),(255,0,0),thickness=1, line_type=cv2.LINE_4, shift=0, tipLength=0.02)
    if abs(int(corner[8])+int(corner[9])) > 10:
        cv2.arrowedLine(self.imgs[img_id], (int(corner[0]),int(corner[1])),(int(corner[8]),int(corner[9])),(0,0,  0),thickness=1, line_type=cv2.LINE_4, shift=0, tipLength=0.02)
    cv2.circle(self.imgs[img_id], (int(corner[0]), int(corner[1])), 3, (255, 0, 0), -1)
  ...
  def save_all_imgs(self, image_name, path='./cache/debug/', prefix='', genID=True):
    if genID:
      try:
