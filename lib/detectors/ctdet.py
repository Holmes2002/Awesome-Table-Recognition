...
from lib.models.decode import ctdet_decode,corner_decode,ctdet_4ps_decode,ctdet_st_decode
from lib.models.utils import flip_tensor
from lib.utils.image import get_affine_transform
from lib.utils.post_process import ctdet_post_process,ctdet_4ps_post_process,ctdet_4ps_post_process_upper_left
from lib.utils.post_process import ctdet_corner_post_process
...
class CtdetDetector(BaseDetector):
  ...
  def process(self, images, origin, return_time=False, batch=None):
    ...
    with torch.no_grad():
      ...
      if batch is None :
        ...
        hm = torch.Tensor(batch['hm']).unsqueeze(0).cpu()
        ...
        wh = wh.view(output['wh'].size(0), output['wh'].size(1), output['wh'].size(2), output['wh'].size(3)).cpu()
        #wh = wh + 2 * torch.rand(size = wh.shape).cpu()
        ...
        reg = reg.view(output['reg'].size(0), output['reg'].size(1), output['reg'].size(2), output['reg'].size(3)).cpu()
  ...
  def show_results(self, debugger, image, results, corner, image_name, logi=None):
    ...
    fc = open("1.txt",'w') #bounding boxes saved
    fv = open("2.txt",'w')
    fl = open("3.txt",'w') #logic axis saved 
    total_bbs = []
    for j in range(1, self.num_classes + 1):
      k = 0
      ...
      if self.opt.vis_corner==1:
        for i in range(m):
          if corner[i,10] > self.opt.vis_thresh_corner:
            ...
            count+=1
    cv2.imwrite('table_img/test_bb.png',image)
    fc.close()
    fv.close() 
    debugger.save_all_imgs(image_name, '/data/disk2/congvu/home1/data/congvu/multilingual-ocr/printed/LORE_result')
    return total_bbs
