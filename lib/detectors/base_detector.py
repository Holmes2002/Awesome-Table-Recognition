...
from lib.models.classifier import Processor, load_processor
from lib.utils.utils import make_batch
...
class BaseDetector(object):
  ...
  def run(self, opt, image, image_anno=None, meta=None):
   
    debugger = Debugger(dataset=self.opt.dataset, ipynb=(self.opt.debug==3),
                        theme=self.opt.debugger_theme)
    image_or_path_or_tensor = 'data/sample.png'
    pre_processed = False
    ...
    for scale in self.scales:
      ...
      if not pre_processed:
        images, meta = self.pre_process(image.copy(), scale, meta)
    ...
    total_bbs =  self.show_results(debugger, image, results, corner_st_reg, image_name, slct_logi.squeeze())
    return total_bbs